---
title: CI/CD GitHub Actions skill
description: A skill to write, edit and review GitHub Actions workflows. MUST USE before editing anything inside a .github directory. 
---
...
3. CI and the dev registry are parts of devs' zone of responsibility. It never touches pre/prod environments or
   registries.
4. CD and the pre/prod registry are parts of ops' zone of responsibility. It never writes to dev registry, only reads
   from it.

In this document it is assumed that GHCR is used as the dev registry and 
`ghcr.io/${{ github.repository }}:${{ github.tag }}` is a production-grade image ready to be deployed.
...
7. Must only pull Docker images from dev registry with `ghcr.io/${{ github.repository }}` path prefix and tags, related
   to the release it was triggered by – either the release tag or commit SHA.
...
jobs:
  ...
  build-and-push:
    ...
    steps: &build-and-push-steps
      ...
      - name: Login to GHCR (dev registry)
...
The split is: **CI writes only to dev registry; CD reads from dev registry and writes to production-grade registries**.
...
graph LR
    ...
    subgraph CD["CD Pipeline - Ops zone"]
        ...
        direction LR
        CD1[retag dev registry image to prod registry] --> CD2[push to prod registry]
        CD2 --> CD3[deploy to preprod]
        CD3 --> CD4[deploy to prod]
        CD-TEST["Test-Project for testing pipelines"]
    end

    PREPRODREGISTRY[pre/prod registry Artifact Registry - pre/prod registry]
    DEVREGISTRY[dev registry - dev registry]
    CI -- 1 . CI Pushes to dev registry --> DEVREGISTRY
    CD -- 2 . CD Retags an image from dev registry --> DEVREGISTRY
    CD -- 3 . CD Pushes an image to pre/prod registry --> PREPRODREGISTRY

```

**CI** workflows run checks and build Docker images to dev registry only. They never authenticate to pre/prod registry
or touch any production environment. 

**CD** workflows (e.g. `cd-pre-release.yml`) pull the CI-built image from dev registry, retag it into the target
environment's Artifact Registry, and deploy to pre/prod. CD is gated behind a release event —
...
1. Developer pushes tag `v1.2.3-rc.1` → `ci.yml` builds and pushes `ghcr.io/<repo>:<sha>` and
   `ghcr.io/<repo>:v1.2.3-rc.1` to dev registry.
2. Developer (or someone from an Ops team) creates a GitHub prerelease from that tag → `cd-pre-release.yml` pulls from
   dev registry, retags to `<preprod-registry>/<repo>:<sha>` and `:<tag>`, and deploys to Cloud Run pre/prod.

dev registry is always the intermediate artifact store. CD never rebuilds from source.
